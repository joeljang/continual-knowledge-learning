...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        self.q_lora = LoRALayer(self.embed_dim, adapter_dim=self.lora_attn_dim, adapter_alpha=self.lora_attn_alpha)
        self.v_lora = LoRALayer(self.embed_dim, adapter_dim=self.lora_attn_dim, adapter_alpha=self.lora_attn_alpha)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            query_delta = self.q_lora(hidden_states)
            value_delta = self.v_lora(encoder_hidden_states)         
            attention_mask = encoder_attention_mask
        else:
            query, key, value = self.c_attn(hidden_states).split(self.split_size, dim=2)
            query_delta = self.q_lora(hidden_states)
            value_delta = self.v_lora(hidden_states)           
